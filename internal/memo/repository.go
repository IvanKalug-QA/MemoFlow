...
func (repo *MemoRepository) Count() int64 {
	var count int64
	repo.Database.Table("memos").Where("deleted_at is null").Count(&count)
	return count
}

func (repo *MemoRepository) GetAll(limit, offset int) []Memo {
	var memos []Memo
